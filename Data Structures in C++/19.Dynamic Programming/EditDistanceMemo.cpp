...
int editDistanceHelper(string s1, string s2, int **output){
  ...
  if(s1[0] == s2[0]){
    ...
    int x = editDistanceHelper(s1.substr(1), s2, output); // Insert
    int y = editDistanceHelper(s1, s2.substr(1), output); // Delete
    int z = editDistanceHelper(s1.substr(1), s2.substr(1), output); // Replace
    ans = 1 + min(x,min(y,z));
  }
  output[m][n] = ans;
  return ans;
...
int editDistance(string s1, string s2){
  ...
  int ans = editDistanceHelper(s1, s2, output);
  for(int i = 0; i <= m; i++){
    delete [] output[i];
  }
  delete [] output;

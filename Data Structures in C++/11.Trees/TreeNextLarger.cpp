...
TreeNode<int>* getNextLargerElement(TreeNode<int>* root, int x) {
    ...
    for(int i = 0; i < root -> children.size(); i++){
      TreeNode<int>* tmp = getNextLargerElement(root -> children[i],x);
      if(ans != NULL){
        if(tmp != NULL && (tmp -> data > x) && ((tmp -> data - x) < (ans -> data - x))){
          ans = tmp;
        }
      }else if(tmp != NULL && (tmp -> data > x)){
        ans = tmp;
      }
    }
    return ans;
...
TreeNode<int>* getNextLargerElement(TreeNode<int>* root, int x) {
    ...
    while(!pendingNodes.empty()){
        ...
        pendingNodes.pop();
        if(front -> data > x && front -> data < nextNum){
            nextNum = front -> data;
            nextLarge = front;
        }

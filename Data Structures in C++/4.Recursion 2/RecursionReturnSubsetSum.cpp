...
int helperSubset(int input[], int n, int startIndex, int output[][50], int k){
  if(startIndex == n){
    if(k == 0){
      output[0][0] = 0;
      return 1;
  ...
  int size1 = helperSubset(input, n, startIndex+1, smallOutput1, k);
  int size2 = helperSubset(input, n, startIndex+1, smallOutput2, (k-input[startIndex]));
...
int subsetSumToK(int input[], int n, int output[][50], int k){
  return helperSubset(input, n, 0, output, k);
}

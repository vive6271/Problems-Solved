...
class Trie {
   ...
   public:
    ...
    bool patternSearch(TrieNode *root, string word){
      if(word.size() == 0){
        return true;
      }
      int index = word[0] - 'a';
      ...
      return patternSearch(child, word.substr(1));
    }

    void patternInsert(string word){
      if(word.size() == 0){
        return;
      }
      insertWord(word);
      patternInsert(word.substr(1));
    }

    bool patternMatching(vector<string> vect, string pattern) {
        // Write your code here
      for(int i = 0; i < vect.size(); i++){
        patternInsert(vect[i]);
      }
